...
export const profileCertificateSchema = z.object({
  name: z.string().min(1, 'Name is required'),                   // Certificate name
  issuingOrganization: z.string().min(1, 'Issuing organization is required'), // Issuer name
  issueDate: z.date({
    required_error: 'Issue Data is required',
    invalid_type_error: 'Invalid date',
  }) ,                                // Date of issuance
  expirationDate: z.date().optional(),                           // Expiration date, if applicable
  credentialId: z.string().optional(),                           // Optional unique credential ID
  credentialUrl: z.string().url('Must be a valid URL').optional(), // Optional URL to the credential
});
...
export const profileEducationSchema = z
  .object({
    ...
    .max(4, { message: "Grade cannot be more than 4" }),
    endDate: z.date({ invalid_type_error: 'Invalid date' }).optional(),
    description: z
    .string()
    .min(50, { message: 'Description must be at least 50 characters' })
...
export type profileCertificateType = z.infer<typeof profileCertificateSchema >;
