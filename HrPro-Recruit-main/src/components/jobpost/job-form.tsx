...
import CustomQuestions, {
  ScreeningQuestion,
} from "./custom-question/CustomQuestions";
import {
  generateAIJobDescription,
  improveText,
} from "@/actions/textgeneration.action";
  ...
  const onSubmit = async (data: JobPostSchemaType) => {
    try {
      ...
      const formattedData = {
        ...
        minExperience: data.hasExperiencerange
          ? Number(data.minExperience)
          : undefined,
        maxExperience: data.hasExperiencerange
  ...
  const askForPrompt = async (): Promise<string> => {
    return new Promise((resolve) => {
      const prompt = window.prompt(
        "Please enter a few words related to the job title or responsibilities:"
      );
      resolve(prompt || "");
    });
  };
  ...
  const generateJobDescription = async () => {
    ...
    try {
      ...
      const context = {
        ...
        experienceRange: formValues.hasExperiencerange
          ? `${formValues.minExperience} - ${formValues.maxExperience} years`
          : "Not specified",
        workMode: formValues.workMode,
        employmentType: formValues.type,
        category: formValues.category,
        salaryRange: formValues.hasSalaryRange
          ? `${formValues.minSalary} - ${formValues.maxSalary} ${formValues.currency}`
          : "Not specified",
        applicationInstructions: formValues.application || " ",
      };
      ...
      form.setValue(
        "responsibilities",
        generatedDescription.responsibilities || []
      );
      form.setValue("skills", generatedDescription.skills || []);
                  ...
                  />
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem className="flex items-center space-y-0 gap-2">
                                ...
                                <FormLabel className="mt-0">
                                  Is there an experience range required for this
                                  role ?
                                </FormLabel>
                              </FormItem>
                            )}
                    ...
                    <div>
                      <div className="flex items-center space-x-4">
                        ...
                        >
                          Want to add your Custom Questions ?
                        </label>
                      </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Job post will be pinned for 30 days.
                  </p>
                </div>
