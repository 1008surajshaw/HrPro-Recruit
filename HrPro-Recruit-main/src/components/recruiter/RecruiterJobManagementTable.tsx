...
import ToggleApproveJobButton from '../admin/ToggleApproveJobButton';
...
const RecruiterJobManagementTable = ({ jobs, searchParams }: props) => {
    ...
    useEffect(() => {
      if (jobs?.jobs) {
        ...
        const searched = filtered.filter((job) => {
          ...
          const lowerCaseCompanyName = job.company.companyName.toLowerCase();
          return (
    ...
    const handleSeeCompanyInfo = (id:string) =>{
      router.push(`jobs/${id}`)
    }
    return (
            ...
            <Table>
              <TableHeader className="bg-gray-100 dark:bg-gray-800 border-b border-gray-800 dark:border-gray-600">
                <TableRow>
                  ...
                  <TableHead>See Job Details</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {currentJobs.map((job) => (
                  <TableRow key={job.id} >
                    <TableCell className="px-2 py-2 cursor-pointer text-red-500" onClick={() =>handleCardClick(job.id)} >{job.title}</TableCell>
                    ...
                    <TableCell className="px-2 py-2 flex  ml-6 cursor-pointer" onClick={()=> handleSeeCompanyInfo(job.id)}>
                        <IoChevronForward  size={16} color='blue'/>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
...
export default RecruiterJobManagementTable