...
const shareOptions: ShareOption[] = [
  {
    ...
    shareFunction: (job: JobType) => {
      const text = encodeURIComponent(
        `Check out this job posting: ${job.title} at ${job.company.companyName}`
      );
  ...
  },
  {
    ...
    shareFunction: (job: JobType) => {
      ...
      const summary = encodeURIComponent(
        `Exciting opportunity: ${job.title} at ${job.company.companyName}. Check out the details!`
      );
      window.open(
  ...
  },
  {
    ...
    shareFunction: (job: JobType) => {
      const text = encodeURIComponent(
        `Check out this job posting: ${job.title} at ${job.company.companyName} - ${window.location.href}`
      );
