...
const NavItem = ({ path, label }: { path: string; label: string }) => {
  ...
  return (
    <Link
      ...
      className={`flex justify-start items-center gap-2 hover:bg-slate-200 hover:border-l-red-700 hover:border-l-2 dark:hover:bg-slate-800 px-3 py-2 transition-colors hover:text-foreground/80 font-medium text-sm  ${pathname === path && 'bg-slate-200 border-l-red-600 border-l-2 dark:bg-slate-800'}`}
    >
      {label}
    </Link>
  );
