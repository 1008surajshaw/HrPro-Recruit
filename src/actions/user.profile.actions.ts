...
export const addUserCertificate = withServerActionAsyncCatcher<
  profileCertificateType,
  ServerActionReturnType
>(async (data) => {
  const auth = await getServerSession(authOptions);
...
export const editCompanyDetails = withServerActionAsyncCatcher<
{data:CompanySchemaType; id:string},
ServerActionReturnType>(async ({data,id}) =>{
  const auth = await getServerSession(authOptions);
...
export const createCompanyDetails = withServerActionAsyncCatcher<
CompanySchemaType,
ServerActionReturnType>(async (data) =>{
  const auth = await getServerSession(authOptions);
...
export const editCertificate = withServerActionAsyncCatcher<
  { data: profileCertificateType; id: number },
  ServerActionReturnType
>(async ({ data, id }) => {
  const auth = await getServerSession(authOptions);
...
export const deleteCertificate = async (certificateId: number) => {
  ...
  try {
    ...
    revalidatePath(`/newProfile/${auth.user.id}`);
    return new SuccessResponse('Certificate Deleted Successfully', 200).serialize();
  } catch (_error) {
    return new ErrorHandler('Internal server error', 'DATABASE_ERROR');
  }
};
