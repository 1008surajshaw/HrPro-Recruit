...
import { WelcomeRecruiterEmail } from '@/mailtemplate/WelcomeRecruiterEmail';
import { WelcomeUserEmail } from '@/mailtemplate/WelcomeUserEmail';
    ...
    await prisma.$transaction(
      async (txn) => {
        ...
        if(data.role === 'HR'){
          await sendEmail(
            data.email,
            "Welcome to HrPro Recruit",
            WelcomeRecruiterEmail(data.name)
          )
        }else{
          await sendEmail(
            data.email,
            "Welcome to HrPro Recruit",
            WelcomeUserEmail(data.name)
          )
        }
