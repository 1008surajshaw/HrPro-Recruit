    ...
    <Sheet>
      ...
      <SheetContent className="w-full">
        <SheetHeader className="h-fit rounded-xl border dark:border-[#1E293B]">
          ...
          <ul className="flex flex-col gap-2 text-sm justify-start items-start px-4 py-2 ">
            ...
            {session.status === 'loading'
                ? nonUserNavbar.map((_, index) => (
                    <Skeleton className="h-4 w-[40px]" key={index} />
                  ))
                : session.data?.user
                ? session.data?.user.role === ADMIN_ROLE ||
                  session.data?.user.role === HR_ROLE
                ...
                {session.status === 'authenticated'
                  && (
                    <div className=''>
                      <div className='transition-colors hover:text-foreground/80 text-foreground/60 font-medium cursor-pointer text-sm py-2 '>
